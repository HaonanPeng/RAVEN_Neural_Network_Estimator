...
class camera_info_definition():
    ...
    ball_move_range_2d = 100.0 #ball center move range in image plane +- (pixel/s)
    ball_move_range_2d_current = 10.0 # = dt * ball_move_range_2d (ball center move range between two frames) 
    
    
class camera_info:
    num_cam = 4  #camera number
    num_ball = 3  #ball number
    list_eff_cam = None # current cam with effective frame number - the frame in which the ball can be found  
    ...
    def detect_locate(self,img_input_list):   
        ...
        for idx_cam in range(self.num_cam):
            ...
            if np.sum(self.cam[idx_cam].img_ball_center)!=0:
                ...
                self.list_eff_cam = np.concatenate((self.list_eff_cam,np.array([idx_cam]))) 
        ...
        if len(self.list_eff_cam)<=1:
            print('effective frame <=1, skip this time step\n')
            return None, img_result_list, self.list_eff_cam
        
        print(self.list_eff_cam)
        self.list_eff_cam = int(self.list_eff_cam)
        
        for idx_cam in range(self.num_cam):
            ball_center_distance = np.array([])
            for idx_ball in range(self.num_ball): 
                for idx_ball_2 in range(idx_ball,self.num_ball):
                    ball_center_distance = np.append(ball_center_distance,np.linalg.norm(ball_center[idx_ball,0:3]-ball_center[idx_ball_2,0:3]))
        ...
        return ball_center, img_result_list, self.list_eff_cam
    ...
    def center_calculator(self,idx_ball):
        if len(self.list_eff_cam) == 2:
            center = self.point_between_2lines(self.cam[self.list_eff_cam[0]].P_cam,self.cam[self.list_eff_cam[1]].P_cam,self.cam[self.list_eff_cam[0]].Cvector[idx_ball],self.cam[self.list_eff_cam[1]].Cvector[idx_ball])
        
        elif len(self.list_eff_cam) == 3:   
            center01 = self.point_between_2lines(self.cam[self.list_eff_cam[0]].P_cam,self.cam[self.list_eff_cam[1]].P_cam,self.cam[self.list_eff_cam[0]].Cvector[idx_ball],self.cam[self.list_eff_cam[1]].Cvector[idx_ball])
            center02 = self.point_between_2lines(self.cam[self.list_eff_cam[0]].P_cam,self.cam[self.list_eff_cam[2]].P_cam,self.cam[self.list_eff_cam[0]].Cvector[idx_ball],self.cam[self.list_eff_cam[2]].Cvector[idx_ball])
            center12 = self.point_between_2lines(self.cam[self.list_eff_cam[1]].P_cam,self.cam[self.list_eff_cam[2]].P_cam,self.cam[self.list_eff_cam[1]].Cvector[idx_ball],self.cam[self.list_eff_cam[2]].Cvector[idx_ball])
            center = (center01+center02+center12)/3
            
        elif len(self.list_eff_cam) == 4:   
            center01 = self.point_between_2lines(self.cam[self.list_eff_cam[0]].P_cam,self.cam[self.list_eff_cam[1]].P_cam,self.cam[self.list_eff_cam[0]].Cvector[idx_ball],self.cam[self.list_eff_cam[1]].Cvector[idx_ball])
            center23 = self.point_between_2lines(self.cam[self.list_eff_cam[3]].P_cam,self.cam[self.list_eff_cam[2]].P_cam,self.cam[self.list_eff_cam[3]].Cvector[idx_ball],self.cam[self.list_eff_cam[2]].Cvector[idx_ball])
