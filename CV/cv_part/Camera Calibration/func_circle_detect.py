...
def circle_center_detect (img, showplot, circle_radius_min=30, circle_radius_max=60):
...
def color_detect4 (target_color, threshhold):
    ...
    if (abs(r-b)<20) & ((g-r)>30) & ((g-b)>40):
        color[0] = 1
    if (abs(r-g)<20) & ((r-b)>60) & ((g-b)>50):
        color[1] = 1
    if ((r-g)>90) & ((r-b)>30) & ((b-g)>40):
        color[2] = 1
...
## old color_detect files
#def color_detect (traget_color, blue_ref, green_ref, red_ref, ref_threshhold):
#    color = np.array((0,0,0))
#    if np.linalg.norm(traget_color - blue_ref) < ref_threshhold:
#        color[0] = 1
#    elif np.linalg.norm(traget_color - green_ref) < ref_threshhold:
#        color[1] = 1
#    elif np.linalg.norm(traget_color - red_ref) < ref_threshhold:
#        color[2] = 1
#    
#    
#    if np.sum(color) > 1:
#        print("[ERROR]: More than one colors are detected, return gray")
#        return np.array((0,0,0))
#    else:
#        return color
#    
#def color_detect2 (target_color, threshhold):
#    color = np.array((0,0,0))
#    if (target_color[0] - target_color[1] > threshhold) & (target_color[0] - target_color[2] > threshhold):
#        color[0] = 1
#    if (target_color[1] - target_color[0] > threshhold) & (target_color[1] - target_color[2] > threshhold):
#        color[1] = 1
#    if (target_color[2] - target_color[0] > threshhold) & (target_color[2] - target_color[1] > threshhold):
#        color[2] = 1
#    
#    
#    if np.sum(color) > 1:
...
#        return color
#    
#def color_detect3 (target_color, threshhold):
#    color = np.array((0,0,0))
#    if (target_color[0] / (target_color[1]+0.1) > threshhold) & (target_color[0] / (target_color[2]+0.1) > threshhold):
#        color[0] = 1
#    if (target_color[1] / (target_color[0]+0.1) > threshhold) & (target_color[1] / (target_color[2]+0.1) > threshhold):
#        color[1] = 1
#    if (target_color[2] / (target_color[0]+0.1) > threshhold) & (target_color[2] / (target_color[1]+0.1) > threshhold):
#        color[2] = 1
#    
#    
#    if np.sum(color) > 1:
