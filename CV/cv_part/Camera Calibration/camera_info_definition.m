...
%object settings-----------------------------------------------------------
num_ref = [9,10,11]; %number of reference objects
P_obj = [127,-127,279.4;              % obj1 (reference) mark0_high
         ...
         -50 29.3 244.475;             % obj9 ball0
         -10 29.3 284.475;             % obj10 ball1
         30	29.3 244.475];            % obj11 ball2
...
for i = 1:num_cam
    fileID = fopen('info_P_chess.txt','a+');
    fprintf(fileID,'%.6f %.6f %.6f\n ',P_chess(i,:));
    fclose(fileID);
    
    fileID = fopen('info_R_chess.txt','a+');
    fprintf(fileID,'%.6f %.6f %.6f\n %.6f %.6f %.6f\n %.6f %.6f %.6f\n',R_chess(:,:,i)');
    fclose(fileID);
    
    fileID = fopen('info_P_cam2chess.txt','a+');
    fprintf(fileID,'%.4f %.4f %.4f\n ',P_cam2chess(i,:));
    fclose(fileID);
    
    fileID = fopen('info_R_cam2chess.txt','a+');
    fprintf(fileID,'%.4f %.4f %.4f\n %.4f %.4f %.4f\n %.4f %.4f %.4f\n',R_cam2chess(:,:,i)');
    fclose(fileID);
    
    fileID = fopen('info_P_cam.txt','a+');
    fprintf(fileID,'%.6f %.6f %.6f\n ',P_cam(i,:));
    fclose(fileID);
    
    fileID = fopen('info_R_cam.txt','a+');
    fprintf(fileID,'%.6f %.6f %.6f\n %.6f %.6f %.6f\n %.6f %.6f %.6f\n',R_cam(:,:,i)');
    fclose(fileID);
end
